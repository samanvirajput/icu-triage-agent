#!/usr/bin/env python3
"""Auto-updates README.md with current project structure and module inventory."""
import os
...
def get_file_list(directory: Path, extensions: tuple = (".py", ".json", ".md", ".txt")) -> list[str]:
    result = []
...
def build_readme() -> str:
    py_files = [f for f in get_file_list(ROOT) if f.endswith(".py")]
    total_lines = sum(count_lines(ROOT / f) for f in py_files)
    ...
    lines = [
        ...
        "```",
        "",
        f"**Python source:** {len(py_files)} files · {total_lines:,} lines",
        "",
        "## Rate Limiting",
        "",
